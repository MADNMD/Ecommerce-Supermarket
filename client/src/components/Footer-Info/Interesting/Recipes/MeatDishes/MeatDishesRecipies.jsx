...
export const MeatDishesRecipies = ({ showNavigationAndFooter }) => {
    ...
    useEffect(() => {
        ...
        const savedLikes = JSON.parse(localStorage.getItem("likedMeanDishesRecipes"));
        if (savedLikes) {
            setLiked(savedLikes);
        }
    }, [showNavigationAndFooter]);
    ...
    const toggleLike = (id) => {
        setLiked(prevLikedStates => {
            ...
            localStorage.setItem("likedMeanDishesRecipes", JSON.stringify(newLikedStates));
    ...
    const currentItems = MeanDishesRecipies.slice(indexOfFirstItem, indexOfLastItem);

    const nextPage = () => {
        if (currentPage < Math.ceil(MeanDishesRecipies.length / itemsPerPage)) {
    ...
    const renderPageNumbers = () => {
        const totalPages = Math.ceil(MeanDishesRecipies.length / itemsPerPage);
        const pageNumbers = [];
    ...
    return (
        <div className={styles['recipies-container']}>
            ...
            <div className={styles.pagination}>
                ...
                {renderPageNumbers()}
                <button className={styles.next} onClick={nextPage} disabled={currentPage === Math.ceil(MeanDishesRecipies.length / itemsPerPage)}><GoChevronRight /></button>
            </div>
        </div>
    )
}