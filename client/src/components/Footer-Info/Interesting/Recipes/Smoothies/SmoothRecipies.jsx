...
export const SmoothRecipies = ({ showNavigationAndFooter }) => {
    ...
    useEffect(() => {
        ...
        const savedLikes = JSON.parse(localStorage.getItem("likedSmoothiesRecipes"));
        if (savedLikes) {
            setLiked(savedLikes);
        }
    }, [showNavigationAndFooter]);
    ...
    const toggleLike = (id) => {
        setLiked(prevLikedStates => {
            ...
            localStorage.setItem("likedSmoothiesRecipes", JSON.stringify(newLikedStates));
    ...
    const currentItems = smoothiesRecipies.slice(indexOfFirstItem, indexOfLastItem);

    const nextPage = () => {
        if (currentPage < Math.ceil(smoothiesRecipies.length / itemsPerPage)) {
    ...
    const renderPageNumbers = () => {
        const totalPages = Math.ceil(smoothiesRecipies.length / itemsPerPage);
        const pageNumbers = [];
    ...
    return (
        <div className={styles['recipies-container']}>
            ...
            <div className={styles.pagination}>
                ...
                {renderPageNumbers()}
                <button className={styles.next} onClick={nextPage} disabled={currentPage === Math.ceil(smoothiesRecipies.length / itemsPerPage)}><GoChevronRight /></button>
            </div>
        </div>
    )
}