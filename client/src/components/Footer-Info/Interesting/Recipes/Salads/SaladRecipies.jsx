...
export const SaladRecipies = ({ showNavigationAndFooter }) => {
    ...
    useEffect(() => {
        ...
        const savedLikes = JSON.parse(localStorage.getItem("likedSaladRecipes"));
        if (savedLikes) {
            setLiked(savedLikes);
        }
    }, [showNavigationAndFooter]);
    ...
    const toggleLike = (id) => {
        setLiked(prevLikedStates => {
            ...
            localStorage.setItem("likedSaladRecipes", JSON.stringify(newLikedStates));
    ...
    const currentItems = saladsRecipies.slice(indexOfFirstItem, indexOfLastItem);

    const nextPage = () => {
        if (currentPage < Math.ceil(saladsRecipies.length / itemsPerPage)) {
    ...
    return (
        <div className={styles['recipies-container']}>
            ...
            <div className={styles.pagination}>
                ...
                {renderPageNumbers()}
                <button className={styles.next} onClick={nextPage} disabled={currentPage === Math.ceil(saladsRecipies.length / itemsPerPage)}><GoChevronRight /></button>
            </div>
        </div>
    )
}