...
export const LeanMealRecipies = ({ showNavigationAndFooter }) => {
    ...
    useEffect(() => {
        ...
        const savedLikes = JSON.parse(localStorage.getItem("likedLeanMealRecipes"));
        if (savedLikes) {
            setLiked(savedLikes);
        }
    }, [showNavigationAndFooter]);
    ...
    const toggleLike = (id) => {
        setLiked(prevLikedStates => {
            ...
            localStorage.setItem("likedLeanMealRecipes", JSON.stringify(newLikedStates));
    ...
    const currentItems = leanMealsRecipies.slice(indexOfFirstItem, indexOfLastItem);

    const nextPage = () => {
        if (currentPage < Math.ceil(leanMealsRecipies.length / itemsPerPage)) {
    ...
    const renderPageNumbers = () => {
        const totalPages = Math.ceil(leanMealsRecipies.length / itemsPerPage);
        const pageNumbers = [];
    ...
    return (
        <div className={styles['recipies-container']}>
            ...
            <div className={styles.pagination}>
                ...
                {renderPageNumbers()}
                <button className={styles.next} onClick={nextPage} disabled={currentPage === Math.ceil(leanMealsRecipies.length / itemsPerPage)}><GoChevronRight /></button>
            </div>
        </div>
    )
}