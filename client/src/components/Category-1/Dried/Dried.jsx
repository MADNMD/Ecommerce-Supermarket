...
import styles from './Dried.module.css';
...
export const Dried = ({showNavigationAndFooter}) => {
    ...
    const currentItems = sortProducts(allDrieds).slice(indexOfFirstItem, indexOfLastItem);

    const nextPage = () => {
        if (currentPage < Math.ceil(allDrieds.length / itemsPerPage)) {
    ...
    const renderPageNumbers = () => {
        const totalPages = Math.ceil(allDrieds.length / itemsPerPage);
        const pageNumbers = [];
            ...
            <div className={styles.container}>
                <div className={styles['fruitAndVeg-main-content']}>
                    ...
                    <div className={styles['fruitAndVeg-content']}>
                        <h3 className={styles['sometimes']}>Сушени плодове и зеленчуци</h3>
                        ...
                        <div className={styles.pagination}>
                            ...
                            {renderPageNumbers()}
                            <button className={styles.next} onClick={nextPage} disabled={currentPage === Math.ceil(allDrieds.length / itemsPerPage)}><GoChevronRight /></button>
                        </div>
                    </div>
                </div>
            </div>
