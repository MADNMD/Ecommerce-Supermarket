...
import styles from './FruitAndVeg.module.css';
...
import { CardThirdItem2 } from "./CardThirdItem2";
import { CardFourItem } from './CardFourItem';
import { CardFiveItem } from "./CardFiveItem";
import { CardFourItem2 } from "./CardFourItem2";

export const FruitAndVeg = ({showNavigationAndFooter}) => {

    const [allFruitAndVegProducts, setAllFruitAndVegProducts] = useState([]);
    ...
    useEffect(() => {
        setLoading(true)
        productService.getAllFruitAndVegProducts()
            .then(products => {
                setAllFruitAndVegProducts(products)
                setLoading(false)
            })
            .catch(error => {
                console.log('Грешка от клиента')
            })
    }, []);
    ...
    const currentItems = sortProducts(allFruitAndVegProducts).slice(indexOfFirstItem, indexOfLastItem);

    const nextPage = () => {
        if (currentPage < Math.ceil(allFruitAndVegProducts.length / itemsPerPage)) {
            ...
            <div className={styles.container}>
                <div className={styles['fruitAndVeg-main-content']}>
                    ...
                    <div className={styles['fruitAndVeg-content']}>
                        <h3>Зеленчуци и плодове</h3>
                        <p className={styles['fruitAndVeg-content-p']}>Ние ще изберем най-свежите и най-качествените плодове и
                            зеленчуци, които ще доставим до прага
                            на вашата врата!
                        </p>
                        <p className={styles['fruitAndVeg-content-p-2']}>Доверете ни се!</p>
                        ...
                        <div className={styles.pagination}>
                            ...
                            {renderPageNumbers()}
                            <button className={styles.next} onClick={nextPage} disabled={currentPage === Math.ceil(allFruitAndVegProducts.length / itemsPerPage)}><GoChevronRight /></button>
                        </div>
                    </div>
                </div>
            </div>
