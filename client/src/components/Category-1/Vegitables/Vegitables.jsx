...
import styles from './Vegitables.module.css';
...
export const Vegitables = ({ showNavigationAndFooter }) => {

    const [allVegitables, setAllVegitables] = useState([]);
    ...
    useEffect(() => {
        setLoading(true);
        productService.getAllVegitables()
            .then(vegitables => {
                setAllVegitables(vegitables);
                setLoading(false);
            })
            .catch(error => {
                console.log(error);
    ...
    const currentItems = sortProducts(allVegitables).slice(indexOfFirstItem, indexOfLastItem);

    const nextPage = () => {
        if (currentPage < Math.ceil(allVegitables.length / itemsPerPage)) {
    ...
    const renderPageNumbers = () => {
        const totalPages = Math.ceil(allVegitables.length / itemsPerPage);
        const pageNumbers = [];
            ...
            <div className={styles.container}>
                <div className={styles['fruitAndVeg-main-content']}>
                    ...
                    <div className={styles['fruitAndVeg-content']}>
                        <h3>Зеленчуци</h3>
                        <p className={styles['fruitAndVeg-content-p']}>Ние ще изберем най-свежите и най-качествените зеленчуци, които ще доставим до прага
                            на вашата врата!
                        </p>
                        ...
                        <div className={styles.pagination}>
                            ...
                            {renderPageNumbers()}
                            <button className={styles.next} onClick={nextPage} disabled={currentPage === Math.ceil(allVegitables.length / itemsPerPage)}><GoChevronRight /></button>
                        </div>
                    </div>
                </div>
            </div>
