...
export const Fruit = ({ showNavigationAndFooter }) => {
    ...
    const currentItems = sortProducts(allFruits).slice(indexOfFirstItem, indexOfLastItem);

    const nextPage = () => {
        if (currentPage < Math.ceil(allFruits.length / itemsPerPage)) {
    ...
    const renderPageNumbers = () => {
        const totalPages = Math.ceil(allFruits.length / itemsPerPage);
        const pageNumbers = [];
            ...
            <div className={styles.container}>
                <div className={styles['fruitAndVeg-main-content']}>
                    ...
                    <div className={styles['fruitAndVeg-content']}>
                        <h3>Плодове</h3>
                        <p className={styles['fruitAndVeg-content-p']}>Ние ще изберем най-свежите и най-качествените плодове, които ще доставим до прага
                            на вашата врата!
                        </p>
                        ...
                        <div className={styles.pagination}>
                            ...
                            {renderPageNumbers()}
                            <button className={styles.next} onClick={nextPage} disabled={currentPage === Math.ceil(allFruits.length / itemsPerPage)}><GoChevronRight /></button>
                        </div>
                    </div>
                </div>
            </div>
