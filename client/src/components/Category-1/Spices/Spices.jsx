...
import * as producService from '../../../services/fruitAndVegService';
...
export const Spices = ({ showNavigationAndFooter }) => {

    const [allSpices, setAllSpices] = useState([]);
    ...
    const currentItems = sortProducts(allSpices).slice(indexOfFirstItem, indexOfLastItem);

    const nextPage = () => {
        if (currentPage < Math.ceil(allSpices.length / itemsPerPage)) {
    ...
    const renderPageNumbers = () => {
        const totalPages = Math.ceil(allSpices.length / itemsPerPage);
        const pageNumbers = [];
            ...
            <div className={styles.container}>
                <div className={styles['fruitAndVeg-main-content']}>
                    ...
                    <div className={styles['fruitAndVeg-content']}>
                        <h3 className={styles['sometimes']}>Свежи Подправки</h3>
                        ...
                        <div className={styles.pagination}>
                            ...
                            {renderPageNumbers()}
                            <button className={styles.next} onClick={nextPage} disabled={currentPage === Math.ceil(allSpices.length / itemsPerPage)}><GoChevronRight /></button>
                        </div>
                    </div>
                </div>
            </div>
